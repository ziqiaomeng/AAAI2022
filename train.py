...
import pandas as pd
from meta_learning import Meta_model

import os
import shutil


def main(dataset, pretrained_gnn, gnn_type,
         add_similarity, add_selfsupervise,
         add_masking, add_weight, m_support,
         add_motif):
    # Training settings
    ...
    parser.add_argument('--pretrained_gnn', type=bool, default=False, help='filename to read the model (if there is any)')
    ...
    # add prototypes
    parser.add_argument('--add_prototype', type=bool, default=True)
    parser.add_argument('--interact', type=bool, default=False)
    parser.add_argument('--add_weight', type=float, default=0.1)
    # add motif
    parser.add_argument('--add_motif', type=bool, default=True)

    args = parser.parse_args()

    args.dataset = dataset
    args.pretrained_gnn = pretrained_gnn
    ...
    for epoch in range(1, args.epochs + 1):
        # get meta-trained model parameters
        ...
        if epoch % 1 == 0:
            ...
            if best_accs != []:
                ...
                best_accs = accs
            # record best result
            fw = open("result/" + args.dataset + "_" + args.gnn_type + "_" + str(args.m_support) + "_" + str(
                args.add_similarity) + "_" + str(args.add_selfsupervise) + "_" + str(args.add_masking) + "_" + str(
                args.add_weight) + "_" + str(args.update_step) + ".txt", "a")
            fw.write("test: " + "\t")
            for i in accs:
                fw.write(str(i) + "\t")
...
if __name__ == "__main__":
    ...
    main("sider", True, "gin", True, True, True, 0.1, 5, True)
